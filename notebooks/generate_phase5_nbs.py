...
def make_nb5(is_smote):
    cells = []
    
    title = "# Phase 5b: Logistic Regression Model (Balanced with SMOTE)" if is_smote else "# Phase 5a: Logistic Regression Model (Imbalanced Data)"
    ...
    cells.append({"cell_type": "markdown", "metadata": {}, "source": ["### Train the Logistic Regression Model\n", "We use `multi_class='multinomial'` to handle the 4 severity levels. `max_iter=1000` ensures the gradient descent algorithm converges on this large dataset."]})
    if is_smote:
        train_code = [
            "print(\"Training Multinomial Logistic Regression Model on balanced data...\")\n",
...
with open('05a_logistic_regression_imbalanced.ipynb', 'w') as f:
    json.dump(make_nb5(False), f, indent=1)
    
with open('05b_logistic_regression_balanced_smote.ipynb', 'w') as f:
    json.dump(make_nb5(True), f, indent=1)
